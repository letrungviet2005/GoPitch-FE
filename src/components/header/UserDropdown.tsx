...
import { Link, useNavigate } from "react-router-dom"; // Sử dụng useNavigate để redirect khi logout
...
export default function UserDropdown() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Giả sử API trả về { name, email, avatar }
        setUser(res.data);
      } catch (error) {
  ...
  const handleSignOut = () => {
    ...
    navigate("/signin"); // Chuyển hướng về trang login
  };
      ...
      >
        ...
        <ul className="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
          <li>
            ...
            >
              {/* SVG Profile... (giữ nguyên của bạn) */}
              Edit profile
            </DropdownItem>
          </li>
